# Translation keys use the original Chinese UI text; keep all translations here.
$script:EnglishText=@{
    '切换效果'='Transition'
    '直接切换'='Instant'
    '淡入淡出'='Crossfade'
    '检查更新…'='Check updates…'
    '检查更新'='Check for updates'
    '下载更新'='Download update'
    '更新并重启'='Update and restart'
    '安装版'='Installed'
    '便携版'='Portable'
    '更新会关闭设置并重启；仅保留已保存的设置，请先保存修改。'='Updating closes Settings and restarts. Only saved settings are retained; save edits first.'
    '正在检查 GitHub 最新正式版本…'='Checking the latest stable release on GitHub…'
    '正在下载并校验更新，关闭窗口可取消…'='Downloading and verifying. Close this window to cancel…'
    '发现新版本：{0} → {1}（{2}）'='Update available: {0} → {1} ({2})'
    '当前 {0}，最新正式版 {1}（{2}），无需更新。'='Current {0}, latest stable {1} ({2}). No update needed.'
    '下载与 SHA-256 校验完成，可以更新。'='Download and SHA-256 verification complete. Ready to update.'
    '更新操作失败：{0}'='Update operation failed: {0}'
    '首次设置引导…'='Setup wizard…'
    '首次设置引导'='Setup wizard'
    '1. 环境与屏幕'='1. Environment and displays'
    '2. 主屏图库'='2. Primary library'
    '3. 副屏图库'='3. Secondary library'
    '4. 轮播与完成'='4. Slideshow and finish'
    '安装目录可写。'='Installation folder is writable.'
    '安装目录不可写。'='Installation folder is not writable.'
    '请先解决以下环境问题：'='Resolve these prerequisite issues first:'
    '主屏以 Windows 设置为准；其他屏幕共用副屏规则。'='Windows chooses the primary display; all other displays share Secondary rules.'
    '只有一块屏幕也可使用；副屏目录默认沿用主屏，可稍后修改。'='One screen also works. Secondary folders default to Primary and can be edited later.'
    '我已确认主副屏分组'='I have checked the Primary / Secondary assignment'
    '完成后保存设置、扫描图库并应用；大图库扫描可能需要几分钟。'='Finish saves settings, scans images and applies wallpapers. Large libraries may take minutes.'
    '取消引导不会保存修改，也不会改变壁纸或轮播任务。'='Cancel leaves settings, wallpapers and slideshow tasks unchanged.'
    '上一步'='Back'
    '下一步'='Next'
    '完成并应用'='Finish and apply'
    '取消'='Cancel'
    '请先解决环境问题，然后重新打开引导。'='Resolve the prerequisite issues, then reopen the wizard.'
    '请先确认主副屏分组。'='Confirm the Primary / Secondary assignment first.'
    '播放顺序'='Order'
    '随机（尽量不连续重复）'='Random (avoid consecutive repeats)'
    '顺序（按文件名排序，循环播放）'='Sequential (sort by filename, repeat)'
    '导入配置…'='Import settings…'
    '导出配置…'='Export settings…'
    '壁纸预览…'='Preview…'
    '壁纸预览'='Wallpaper preview'
    '配置已载入，尚未保存；请检查后保存并应用。'='Settings loaded, not saved. Review, then save and apply.'
    '以下目录不存在，请检查：{0}'='These folders do not exist. Please review: {0}'
    '请选择其他文件，不能覆盖当前配置。'='Choose another file; the active configuration cannot be overwritten.'
    '配置已导出。'='Settings exported.'
    '请选择示例图片'='Select a sample image'
    '没有可预览的显示器。'='No connected displays to preview.'
    '选择示例图片…'='Choose sample image…'
    '示例预览，不代表下一张轮播图片；平铺按单屏示意。'='Sample preview, not the next slideshow image. Tile is illustrated per screen.'
    '使用此显示方式'='Use this display mode'
    '关闭'='Close'
    '图片无法读取：{0}'='Could not read image: {0}'
    '部分图片无法读取，请重新选择示例图片。'='Some images could not be read. Choose another sample image.'
    '以下项目不是有效目录：{0}'='These items are not valid folders: {0}'
    '分辨率预设'='Resolution preset'
    '自定义'='Custom'
    '可拖入目录；关闭过滤保留参数；删除整行可移除目录。'='Drop folders here. Disabled filters keep values. Delete a line to remove a folder.'
    '分钟'='minutes'
    '显示方式'='Display mode'
    '填充（等比铺满，裁剪边缘）'='Fill (keep ratio, crop edges)'
    '适应（完整显示，可能留边）'='Fit (whole image, may leave borders)'
    '拉伸（铺满，可能变形）'='Stretch (cover screen, may distort)'
    '居中（原始尺寸，可能裁剪）'='Center (original size, may crop)'
    '平铺（原始尺寸，重复排列）'='Tile (repeat at original size)'
    '双屏壁纸 v{0} · 设置'='DualScreenWallpaper v{0} - Settings'
    '语言 / Language'='语言 / Language'
    '选择目录（包含其全部子目录）'='Select a folder (including all subfolders)'
    '添加目录…'='Add folder…'
    '图片目录（每行一个绝对路径，递归扫描）'='Image folders (one absolute path per line; includes subfolders)'
    '排除目录 / Exclude（可留空，该目录及全部子目录都不扫描）'='Excluded folders (optional; skips each folder and all its subfolders)'
    '启用方向过滤'='Filter by orientation'
    '仅横图（宽 > 高）'='Landscape only (width > height)'
    '仅竖图（高 > 宽）'='Portrait only (height > width)'
    '启用最低分辨率'='Minimum resolution'
    '宽'='W'
    '高'='H'
    '像素；0 表示不限该边'='Pixels; 0 means no limit'
    '关闭过滤会保留参数；删除目录所在行即可移除。'='Disabled filters keep their values. Delete a line to remove a folder.'
    '主屏以 Windows“主显示器”为准；其他显示器使用副屏设置，不随横竖方向切换。'='Windows primary display uses Primary; all others use Secondary, regardless of rotation.'
    '主屏'='Primary'
    '副屏'='Secondary'
    '切换间隔'='Interval'
    '分钟     显示方式：填充（铺满屏幕，允许裁剪）'='minutes     Display mode: Fill (covers the screen; may crop edges)'
    '启用自动轮播，并在 Windows 登录后自动启动'='Enable automatic slideshow and start when signing in to Windows'
    '旧配置已载入：横屏 → 主屏，竖屏 → 副屏。请核对后保存；旧配置将备份。'='Legacy settings: Landscape → Primary, Portrait → Secondary. Review and save; backup is automatic.'
    '取消自动轮播后，“保存并应用”仅换图一次，同时关闭自动轮播。'='With slideshow disabled, Save and apply changes wallpapers once and stops automatic changes.'
    '就绪。首次扫描大量图片可能需要几分钟。'='Ready. The first scan of a large library may take several minutes.'
    '主屏和副屏各至少需要一个图片目录。'='Primary and Secondary each need at least one image folder.'
    '图片目录不存在：{0}'='Image folder does not exist: {0}'
    '正在扫描或换图，请等当前操作完成后再保存。'='A scan or wallpaper change is running. Wait before saving.'
    '请等待当前操作完成。'='Please wait for the current operation to finish.'
    '正在处理… 扫描完成后会显示结果。'='Working… Results will appear when the operation finishes.'
    '操作未完成'='Operation could not be completed'
    '保存并应用'='Save and apply'
    '刷新图片索引'='Refresh index'
    '立即换图'='Change now'
    '停止并恢复壁纸'='Stop and restore'
    '操作完成。'='Operation completed.'
    '操作完成。合格图片：主屏 {0} 张，副屏 {1} 张。'='Completed. Eligible images: Primary {0}, Secondary {1}.'
    '操作失败，请查看下方详情；配置仍可修改后重试。'='Operation failed. See the details below; edit your settings and try again.'
    '读取进度失败：{0}'='Could not read progress: {0}'
    '当前操作尚未完成，请稍候再关闭窗口。'='An operation is still running. Please wait before closing.'
}
function Get-UiText([string]$key, [object[]]$values=@()) {
    $text=$key
    if($script:uiLanguage -eq 'en-US' -and $script:EnglishText.ContainsKey($key)){$text=$script:EnglishText[$key]}
    if($values.Count){return ($text -f $values)}
    return $text
}
function Save-UiLanguage([string]$path,[string]$language) {
    if($language -notin 'zh-CN','en-US'){throw 'Unsupported language'}
    $lock=[Threading.Mutex]::new($false,'Local\DualScreenWallpaperWorker'); $held=$false
    try {
        try {$held=$lock.WaitOne(0)} catch [Threading.AbandonedMutexException] {$held=$true}
        if(!$held){throw (Get-UiText '正在扫描或换图，请等当前操作完成后再保存。')}
        # Save only the preference: do not overwrite unsaved controls or migrate legacy settings.
        $settings=Get-Content -LiteralPath $path -Raw -Encoding UTF8 | ConvertFrom-Json
        $settings | Add-Member -NotePropertyName Language -NotePropertyValue $language -Force
        $settings | ConvertTo-Json -Depth 6 | Set-Content -LiteralPath ($path+'.tmp') -Encoding UTF8
        Move-Item -LiteralPath ($path+'.tmp') -Destination $path -Force
    } finally {if($held){$lock.ReleaseMutex()}; $lock.Dispose()}
}
